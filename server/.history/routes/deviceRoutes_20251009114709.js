...
router.post('/:id/upload-images', verifyToken, upload.array('images', 5), async (req, res) => {
  ...
  // ดึง path ของรูปทั้งหมด
  const imagePaths = req.files.map(file => `/uploads/${file.filename}`);

  try {
    // เรียกฟังก์ชันอัปเดตรูป (ให้รองรับหลายรูป)
    await deviceController.updateDeviceImages(deviceId, imagePaths);

    // ดึงข้อมูลล่าสุด
    const [device] = await deviceController.getDeviceById(deviceId);
    res.json({ message: 'อัปโหลดรูปสำเร็จ', device: device[0] });
  } catch (err) {
    console.error('Error uploading device images:', err);
